	...
	if (user)
	{
		if (user->m_isTracked)
		{
			...
			if (xjoint == XN_SKEL_WAIST) // special case currently not supported by OpenNI but approximated as center between hips
			{
				...
				cap.GetSkeletonJointPosition(user->m_id, XN_SKEL_LEFT_HIP, pos1);
				cap.GetSkeletonJointPosition(user->m_id, XN_SKEL_RIGHT_HIP, pos2);
				...
				cap.GetSkeletonJointOrientation(user->m_id, XN_SKEL_TORSO, rot);
			...
			else
			{
				...
				XnSkeletonJointOrientation rot;
				cap.GetSkeletonJointPosition(user->m_id, xjoint, pos);
				cap.GetSkeletonJointOrientation(user->m_id, xjoint, rot);
		...
		else
		{
			...
			XnPoint3D com;
			m_UserGenerator.GetCoM(user->m_id, com);
...
const unsigned short* FubiOpenNISensor::getUserLabelData()
{
...
Fubi::Vec3f FubiOpenNISensor::realWorldToProjective(const Fubi::Vec3f& realWorldVec)
{
	...
	{
		XnPoint3D pos;
		pos.X = realWorldVec.x;
		pos.Y = realWorldVec.y;
		pos.Z = realWorldVec.z;
