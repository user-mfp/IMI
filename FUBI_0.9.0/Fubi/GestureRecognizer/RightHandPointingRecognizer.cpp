...
Fubi::RecognitionResult::Result  RightHandPointingRecognizer::recognizeOn(FubiUser* user, Fubi::RecognitionCorrectionHint* correctionHint /*= 0x0*/)
{
	...
	if (rightHand.m_confidence >= m_minConfidence && rightShoulder.m_confidence >= m_minConfidence)
	{
		...
		else if (user->m_bodyMeasurements[BodyMeasurement::UPPER_ARM_LENGTH].m_confidence >= m_minConfidence)
		{
			armStretched = dir.length() > user->m_bodyMeasurements[BodyMeasurement::UPPER_ARM_LENGTH].m_dist;
		}
		else
		{
