...
FubiKinectSDKSensor::FubiKinectSDKSensor() : m_pNuiSensor(NULL), 
	...
	m_videoBuffer(NULL), m_convertedVideoBuffer(NULL), m_depthBuffer(NULL), m_convertedDepthBuffer(NULL), m_registeredDepthBuffer(NULL), m_irBuffer(0x0),
...
bool FubiKinectSDKSensor::initWithOptions(const Fubi::SensorOptions& options)
{
	...
	else
	{
		...
		for (int i = 0; i < iSensorCount; ++i)
		{
			...
			HRESULT hr = NuiCreateSensorByIndex(i, &m_pNuiSensor);
			if (SUCCEEDED(hr))
			{
				// Check if usable
				...
				if (hr == S_OK)
				{
					HRESULT hr = m_pNuiSensor->NuiInitialize(dwNuiInitDepthFlag | NUI_INITIALIZE_FLAG_USES_SKELETON | NUI_INITIALIZE_FLAG_USES_COLOR);
					if (SUCCEEDED(hr))
						break;
					else
						m_pNuiSensor = nullptr;
...
void FubiKinectSDKSensor::gotDepthAlert( )
{
	...
	if (SUCCEEDED(pTexture->LockRect(0, &LockedRect, NULL, 0)))
	{
		if (LockedRect.Pitch)
		{
			...
			{
				registerDepthData((USHORT*)m_depthBuffer->GetBuffer(), (USHORT*)m_registeredDepthBuffer->GetBuffer(),
...
void FubiKinectSDKSensor::registerDepthData(USHORT* srcBuffer, USHORT* dstBuffer, int depthWidth, int depthHeight, int colorWidth, int colorHeight)
{
	if (SUCCEEDED(m_pNuiSensor->NuiImageGetColorPixelCoordinateFrameFromDepthPixelFrameAtResolution(
		m_colorRes, m_depthRes, depthWidth*depthHeight, srcBuffer, depthWidth*depthHeight*2, m_depthToColorMap)))
	{
		USHORT z, fullZ;
		LONG mapX, mapY;
		int depthIndex;
		...
		for (LONG y=0; y < colorHeight; ++y)
		{
			...
			{
				depthIndex = (int)(x/m_colorToDepthDivisor + y/m_colorToDepthDivisor * depthWidth);
...
Fubi::Vec3f FubiKinectSDKSensor::realWorldToProjective(const Fubi::Vec3f& realWorldVec)
{
	...
	// Register to color TODO: doesn't work as expected
	/*if (m_options.m_rgbOptions.isValid() && m_options.m_registerStreams
	&& ret.x >= 0 && ret.x < m_options.m_depthOptions.m_width && ret.y >= 0 && ret.y < m_options.m_depthOptions.m_height)
	{
	int depthIndex = (int) (ret.x + ret.y * m_options.m_depthOptions.m_width);
	LONG mapX = m_depthToColorMap[depthIndex * 2];
	LONG mapY = m_depthToColorMap[depthIndex * 2 + 1];
	ret.x = (float)mapX;
	ret.y = (float)mapY;
	}*/
