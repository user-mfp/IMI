...
b) Kinect for Xbox/Windows: See 4. Kinect SDK 1.x
...
Add the Leap lib folder to your path, that should be similar to: C:\LeapSDK\lib
Set the include and lib paths for the Leap SDK: In Visual Studio 2010, navigate to View => Property Manager => "Microsoft.Cpp.Win32.user"  under a random (yes, really!) project and config => Select VC++-directories => Add the include and lib path of your OpenCV installation to the corresponding properties. Alternatively you find the related config file “Microsoft.Cpp.Win32.user.props” in "%LocalAppData%\Microsoft\MSBuild\v4.0" that you can edit with a text editor and add the relevant paths. This file should then look something like this:
...
6. We recommend to additionally install OpenCV, currently, the latest supported version is 2.4.7:
For Windows: https://sourceforge.net/projects/opencvlibrary/files/opencv-win/2.4.7/OpenCV-2.4.7.exe/download
If you do not want to use OpenCV, comment out the line "#define USE_OPENCV" at top of the FubiConfig.h
Add the OpenCV bin folder to your path, that should be similar to: C:\OpenCV2.4.7\build\x86\vc10\bin
Set the include and lib paths for OpenCV: In Visual Studio 2010, navigate to View => Property Manager => "Microsoft.Cpp.Win32.user"  under a random (yes, really!) project and config => Select VC++-directories => Add the include and lib path of your OpenCV installation to the corresponding properties. Alternatively you find the related config file “Microsoft.Cpp.Win32.user.props” in "%LocalAppData%\Microsoft\MSBuild\v4.0" that you can edit with a text editor and add the relevant paths. This file should then look something like this:
    <?xml version="1.0" encoding="utf-8"?>
    <Project DefaultTargets="Build" ToolsVersion="4.0" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
      <PropertyGroup>
        <IncludePath>C:\OpenCV2.4.7\build\include;$(IncludePath)</IncludePath>
      </PropertyGroup>
      <PropertyGroup>
        <LibraryPath>C:\OpenCV2.4.7\build\x86\vc10\lib;$(LibraryPath)</LibraryPath>
      </PropertyGroup>
    </Project>
Fubi automatically checks for the installed OpenCV version and links against it. However, if you have different versions installed and you can select one by uncommenting the line "#define FUBI_OPENCV_VERSION 247" and change the 247 to the version that you would like to use.

 
Running the basic FUBI sample
=============================
1. Open the FUBI.sln
2. If needed, edit the "FubiConfig.h" to be found under the project "Fubi" --> header files.
...
It further provides a very good way to test your recognizers, as it provides almost everything that you can adjust in FUBI and further visualizes a lot of useful information about the Fubi functionality.
...
Both samples are tested with OpenNI 2.2, NITE 2.2, OpenCV 2.4.7, as well as the Kinect SDK 1.8 and the Leap Motion SDK 1.0.9
...
Implementation of recognizer in code inside of Fubi
===================================================
We don't recommend it anymore (you get into trouble when updating to a new Fubi version, and you always need to recompile Fubi after changing a recognizer), but it is still possible to implement recognizers directly in FUBI.
