...
namespace Fubi
{
	...
	* @param userID the id of the user to be checked
	* @param correctionHint [out] will contain information about why the recognition failed if wanted
	* @return RECOGNIZED in case of a succesful detection, TRACKING_ERROR if a needed joint is currently not tracked, NOT_RECOGNIZED else
	*/
	...
	* @param correctionHint [out] on NOT_RECOGNIZED, this struct will contain information about why the recognition failed if wanted
	* @return RECOGNIZED in case of a succesful detection, TRACKING_ERROR if a needed joint is currently not tracked, NOT_RECOGNIZED else
	*/
	FUBI_API Fubi::RecognitionResult::Result getCombinationRecognitionProgressOn(Combinations::Combination combinationID, unsigned int userID, 
		std::vector<FubiUser::TrackingData>* userStates = 0x0, bool restart = true, bool returnFilteredData = false, Fubi::RecognitionCorrectionHint* correctionHint = 0x0);
	...
	* @param minX, minY, minZ (=-inf, -inf, -inf) the minimal values allowed for the vectore relJoint -> joint
	* @param maxX, maxY, maxZ (=inf, inf, inf) the maximal values allowed for the vectore relJoint -> joint
	...
	* @param recognizerName name of the combination
	* @param handId of the hand to be checked
	* @param handStates (= 0x0) [out] pointer to a vector of tracking data that represents the tracking information of the user
	*		  during the recognition of each state
	* @param restart (=true) if set to true, the recognizer automatically restarts, so the combination can be recognized again.
	* @param returnFilteredData if true, the user states vector will contain filtered data
	* @param correctionHint [out] on NOT_RECOGNIZED, this struct will contain information about why the recognition failed if wanted
	...
	*/
	FUBI_API Fubi::RecognitionResult::Result getCombinationRecognitionProgressOnHand(const char* recognizerName, unsigned int handId,
		std::vector<FubiHand::FingerTrackingData>* handStates = 0x0, bool restart = true, bool returnFilteredData = false, Fubi::RecognitionCorrectionHint* correctionHint = 0x0);
