	...
	unsigned int userId /*= 0*/, Fubi::SkeletonJoint::Joint jointOfInterest /*= Fubi::SkeletonJoint::NUM_JOINTS*/, bool moveCroppedToUpperLeft /*=false*/)
{
	...
	{
		success = drawIRImage(sensor, outputImage, numChannels, depth);
	...
	{
		drawTrackingInfo(sensor, outputImage, width, height, numChannels, depth, renderOptions, jointsToRender);
	}
	...
	if (success && userId != 0)
	{
		...
		success = setROIToUserJoint(image, userId, jointOfInterest, applyThreshold, (renderOptions&RenderOptions::UseFilteredValues)!=0);
		if (success)
		{
			// Get the set roi
			...
			{
				// Copy out roi subimage
				IplImage* subImage = cvCreateImage(cvSize(roiRect.width, roiRect.height), (depth == ImageDepth::D8) ? IPL_DEPTH_8U : IPL_DEPTH_16U, numChannels); 
				cvCopy(image, subImage);
				// Set whole image to zeros
				cvResetImageROI(image);
				cvZero(image);
				// And copy back the cropped image at the upper left corner
				cvSetImageROI(image, cvRect(0, 0, roiRect.width, roiRect.height));
				cvCopy(subImage, image);
				cvResetImageROI(image);
				cvReleaseImage(&subImage);
			}
			else // Simply fill everything else than the roi with zeros
			{
				// Reset roi
				cvResetImageROI(image);
...
void FubiImageProcessing::drawTrackingInfo(FubiISensor* sensor, unsigned char* outputImage, int width, int height, ImageNumChannels::Channel numChannels, ImageDepth::Depth depth, int renderOptions, int jointsToRender)
{
	FubiUser** users;
		...
		for (unsigned short i = 0; i < numUsers; i++)
		{
				...
				&& users[i]->m_currentTrackingData->jointPositions[SkeletonJoint::TORSO].m_position.z > 100.0f)
			{
				...
				if (users[i]->m_isTracked)
				{
					...
					| RenderOptions::GlobalPosCaptions | RenderOptions::LocalPosCaptions))
					{
						...
						if (jointsToRender & RenderOptions::LEFT_WRIST)
							drawLimb(users[i]->m_id, SkeletonJoint::LEFT_ELBOW, SkeletonJoint::LEFT_WRIST, outputImage, width, height, numChannels, depth, renderOptions);
						if (jointsToRender & RenderOptions::LEFT_HAND)
							drawLimb(users[i]->m_id, SkeletonJoint::LEFT_WRIST, SkeletonJoint::LEFT_HAND, outputImage, width, height, numChannels, depth, renderOptions);

						if (jointsToRender & RenderOptions::RIGHT_SHOULDER)
							drawLimb(users[i]->m_id, (jointsToRender & RenderOptions::NECK) ? SkeletonJoint::NECK : SkeletonJoint::LEFT_SHOULDER, SkeletonJoint::RIGHT_SHOULDER, outputImage, width, height, numChannels, depth, renderOptions);
						...
						if (jointsToRender & RenderOptions::RIGHT_WRIST)
							drawLimb(users[i]->m_id, SkeletonJoint::RIGHT_ELBOW, SkeletonJoint::RIGHT_WRIST, outputImage, width, height, numChannels, depth, renderOptions);
						if (jointsToRender & RenderOptions::RIGHT_HAND)
							drawLimb(users[i]->m_id, SkeletonJoint::RIGHT_WRIST, SkeletonJoint::RIGHT_HAND, outputImage, width, height, numChannels, depth, renderOptions);
						...
						if (jointsToRender & RenderOptions::LEFT_ANKLE)
							drawLimb(users[i]->m_id, SkeletonJoint::LEFT_KNEE, SkeletonJoint::LEFT_ANKLE, outputImage, width, height, numChannels, depth, renderOptions);
						if (jointsToRender & RenderOptions::LEFT_FOOT)
							drawLimb(users[i]->m_id, SkeletonJoint::LEFT_ANKLE, SkeletonJoint::LEFT_FOOT, outputImage, width, height, numChannels, depth, renderOptions);
						...
						if (jointsToRender & RenderOptions::RIGHT_ANKLE)
							drawLimb(users[i]->m_id, SkeletonJoint::RIGHT_KNEE, SkeletonJoint::RIGHT_ANKLE, outputImage, width, height, numChannels, depth, renderOptions);
						if (jointsToRender & RenderOptions::RIGHT_FOOT)
							drawLimb(users[i]->m_id, SkeletonJoint::RIGHT_ANKLE, SkeletonJoint::RIGHT_FOOT, outputImage, width, height, numChannels, depth, renderOptions);

						if (jointsToRender & RenderOptions::WAIST)
						{
							drawLimb(users[i]->m_id, SkeletonJoint::LEFT_HIP, SkeletonJoint::WAIST, outputImage, width, height, numChannels, depth, renderOptions);
				...
				if (renderOptions & RenderOptions::UserCaptions)
				{
					...
					FubiUser::TrackingData* data = users[i]->m_currentTrackingData;
					if (renderOptions & RenderOptions::UseFilteredValues)
					{
						data = users[i]->m_filteredTrackingData;
					}
					...
					posProjective.x *= depthToImageScale.x;
					posProjective.y *= depthToImageScale.y;
			...
			if (hand)
			{
				if (hand->m_isTracked)
				{
					...
					| RenderOptions::GlobalPosCaptions | RenderOptions::LocalPosCaptions))
					{
						...
						/*if (jointsToRender & RenderOptions::PALM)
							drawFinger(hand->m_id, SkeletonHandJoint::PALM, outputImage, width, height, numChannels, depth, renderOptions);*/
						if (jointsToRender & RenderOptions::FINGER_ONE)
							drawFinger(hand->m_id, SkeletonHandJoint::FINGER_ONE, outputImage, width, height, numChannels, depth, renderOptions);
						if (jointsToRender & RenderOptions::FINGER_TWO)
							drawFinger(hand->m_id, SkeletonHandJoint::FINGER_TWO, outputImage, width, height, numChannels, depth, renderOptions);
						if (jointsToRender & RenderOptions::FINGER_THREE)
							drawFinger(hand->m_id, SkeletonHandJoint::FINGER_THREE, outputImage, width, height, numChannels, depth, renderOptions);
						if (jointsToRender & RenderOptions::FINGER_FOUR)
							drawFinger(hand->m_id, SkeletonHandJoint::FINGER_FOUR, outputImage, width, height, numChannels, depth, renderOptions);
						if (jointsToRender & RenderOptions::FINGER_FIVE)
							drawFinger(hand->m_id, SkeletonHandJoint::FINGER_FIVE, outputImage, width, height, numChannels, depth, renderOptions);
					}
				}
				...
				if (renderOptions & RenderOptions::UserCaptions)
				{
					...
					if (renderOptions & RenderOptions::LocalOrientCaptions)
					{
						...
						ss << "Hand"  << hand->m_id << "@(" << rot.x << "," << rot.y << "," << rot.z;
					}
					...
					else if (renderOptions & RenderOptions::LocalPosCaptions)
					{
						...
						ss << "Hand"  << hand->m_id << "@(" << jPos.x << "," << jPos.y << "," << jPos.z;
					}
					else
					{
						ss << "Hand"  << hand->m_id << "@(" << pos.x << "," << pos.y << "," << pos.z;
					}
					...
					// print text
					float r, g, b;
					getColorForUserID(hand->m_id, r, g, b);
					if (renderOptions & RenderOptions::SwapRAndB)
						swap(r, b);
					double maxValue = (depth == ImageDepth::D16) ? Math::MaxUShort16 : 255;
					int offset = int(3.5f * label.length());
					...
					if (posProjective.x >= 0 && posProjective.x < width && posProjective.y >= 0 && posProjective.y < height)
					{
						// Now print it (opposite color)
						double hScale=0.5*depthToImageScale.x;
						double vScale=0.55*depthToImageScale.y;
	...
	if (sensor)
	{
		...
		if (type == ImageType::Color)
		{
			...
			if (options.isValid())
			{
				...
				success = drawColorImage(sensor, (unsigned char*)(image->imageData), numChannels, depth, (renderOptions & RenderOptions::SwapRAndB) == 0);
			}
		}
		else if (type == ImageType::IR)
		{
			...
			{
				image = cvCreateImage(cvSize(options.m_width, options.m_height), depth, numChannels);
				success = drawIRImage(sensor, (unsigned char*)(image->imageData), numChannels, depth);
			}
		}
		else
		{
			...
			if (options.isValid())
			{
				...
				success = drawDepthImage(sensor, (unsigned char*)(image->imageData), numChannels, depth, depthModifications, renderOptions);
			}
		}

		if (success && userId != 0)
		{
			success = success && setROIToUserJoint(image, userId, jointOfInterest, applyThreshold, (renderOptions&RenderOptions::UseFilteredValues)!=0);
		}

		if (success && (renderOptions != RenderOptions::None))
		{

			drawTrackingInfo(sensor, (unsigned char*)(image->imageData), options.m_width, options.m_height, numChannels, depth, renderOptions, jointsToRender);
		}
		...
		if (success)
		{
			...
			success = cvSaveImage(fileName, image, quality) != 0;
		}
...
void FubiImageProcessing::drawLimb(unsigned int userID, Fubi::SkeletonJoint::Joint eJoint1, Fubi::SkeletonJoint::Joint eJoint2, unsigned char* outputImage, int width, int height, 
	Fubi::ImageNumChannels::Channel numChannels, Fubi::ImageDepth::Depth depth, int renderOptions)
{
#ifdef FUBI_USE_OPENCV
	FubiUser* user = Fubi::getUser(userID);
	if (user && user->m_isTracked)
	{
		...
		// Get positions
		SkeletonJointPosition& joint1 = data->jointPositions[eJoint1];
		SkeletonJointPosition& joint2 = data->jointPositions[eJoint2];
...
void FubiImageProcessing::drawFinger(unsigned int handId, Fubi::SkeletonHandJoint::Joint finger, unsigned char* outputImage, int width, int height,
		Fubi::ImageNumChannels::Channel numChannels, Fubi::ImageDepth::Depth depth, int renderOptions)
{
#ifdef FUBI_USE_OPENCV
	FubiCore* core = FubiCore::getInstance();
	if (core)
	{
		...
		if (hand && hand->m_isTracked)
		{
			...
			// Get positions
			SkeletonJointPosition& fingerPos = data->jointPositions[finger];
			SkeletonJointPosition& palmPos = data->jointPositions[SkeletonHandJoint::PALM];
			...
			if (fingerPos.m_confidence > 0)
			{
				...
				// Convert to projective (screen coordinates)
				Fubi::Vec3f pos2 = Fubi::realWorldToProjective(fingerPos.m_position);
				Fubi::Vec3f pos1 = Fubi::realWorldToProjective(palmPos.m_position);
				...
				if (pos2.x >= 0 && pos2.x < width && pos2.y >= 0 && pos2.y < height)
				{
					...
					double hScale=0.4*depthToImageScale.x;
					double vScale=0.45*depthToImageScale.y;
					int    thickness=(int)depthToImageScale.y;

					if (renderOptions & RenderOptions::Skeletons)
					{
						// Draw finger
						cvCircle(image, cvPoint((int)pos2.x, (int)pos2.y), 5000 / (int)clamp(pos2.z, 1.0f, Math::MaxFloat), cvScalar(a*(1-b), a*(1-g), a*(1-r), a), -1);
						if (finger != SkeletonHandJoint::PALM && palmPos.m_confidence > 0.25f && pos1.x >= 0 && pos1.x < width && pos1.y >= 0 && pos1.y < height)
						{
							double lineA = a;
...
bool FubiImageProcessing::setROIToUserJoint(void* pImage, unsigned int userId, Fubi::SkeletonJoint::Joint jointOfInterest, int applyThreshold /*= 0*/, bool useFilteredData /*= false*/)
{
	bool foundRoi = false;
#ifdef FUBI_USE_OPENCV
	...
	if (Fubi::currentTime() - lastWarning > 10)
	{
		Fubi_logWrn("Sorry, can't process image without FUBI_USE_OPENCV defined in the FubiConfig.h.\n");
		lastWarning = Fubi::currentTime();
	}
#endif
...
void FubiImageProcessing::applyThreshold(void* pImage, unsigned int min, unsigned int max, unsigned int replaceValue /*= 0*/)
{
	...
	if (Fubi::currentTime() - lastWarning > 10)
	{
		Fubi_logWrn("Sorry, can't apply threshold on image without FUBI_USE_OPENCV defined in the FubiConfig.h.\n");
		lastWarning = Fubi::currentTime();
	}
#endif
}
...
int FubiImageProcessing::applyFingerCount(FubiISensor* sensor, unsigned int userID, bool leftHand /*= false*/, bool useOldConvexityDefectMethod /*= false*/, FingerCountImageData* debugData /*= 0x0*/)
{
	...
	if (sensor)
	{
		...
		if (setROIToUserJoint(image, userID, leftHand ? SkeletonJoint::LEFT_HAND : SkeletonJoint::RIGHT_HAND, 75, true))
		{
