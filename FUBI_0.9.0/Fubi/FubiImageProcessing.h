...
private:
	...
	static bool drawDepthImage(FubiISensor* sensor, unsigned char* outputImage, Fubi::ImageNumChannels::Channel numChannels, Fubi::ImageDepth::Depth depth, Fubi::DepthImageModification::Modification depthModifications, int renderOptions);
	// Adds tracking info to a image
	static void drawTrackingInfo(FubiISensor* sensor, unsigned char* outputImage, int width, int height, Fubi::ImageNumChannels::Channel numChannels, Fubi::ImageDepth::Depth depth, int renderOptions, int jointsToRender);
	...
	static void drawLimb(unsigned int userId, Fubi::SkeletonJoint::Joint eJoint1, Fubi::SkeletonJoint::Joint eJoint2, unsigned char* outputImage, int width, int height,
	...
	static void drawBodyMeasurement(unsigned int userId, Fubi::SkeletonJoint::Joint eJoint1, Fubi::SkeletonJoint::Joint eJoint2, Fubi::BodyMeasurement::Measurement bodyMeasure, unsigned char* outputImage, int width, int height, 
	Fubi::ImageNumChannels::Channel numChannels, Fubi::ImageDepth::Depth depth, int renderOptions);

	// Draw a finger in the image buffer
	static void drawFinger(unsigned int handId, Fubi::SkeletonHandJoint::Joint finger, unsigned char* outputImage, int width, int height,
		Fubi::ImageNumChannels::Channel numChannels, Fubi::ImageDepth::Depth depth, int renderOptions);
	...
	static bool setROIToUserJoint(void* pImage, unsigned int userId, Fubi::SkeletonJoint::Joint jointOfInterest, int applyThreshold = 0, bool useFilteredData = false);
